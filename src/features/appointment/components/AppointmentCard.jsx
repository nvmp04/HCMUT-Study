...
import { 
  Clock, User, MapPin, Video, FileText, 
  Trash, MonitorUp, Paperclip, Edit, Zap, GraduationCap
} from 'lucide-react';
            ...
            {role === "tutor" ? (
              <>
                ...
                <span className="uppercase tracking-tight">
                  {appointment.studentName || "Học viên"}
                </span>
              </>
            ) : (
              <>
                ...
                <span className="uppercase tracking-tight">
                  {appointment.tutorName || "Giảng viên"}
                </span>
              </>
            )}
          </div>
        ...
        <div className="p-6 flex items-center justify-end bg-black/10 md:bg-transparent min-w-[200px]">
          ...
          {status === "accepted" && (
            <div className="flex gap-2 items-center">
              <button onClick={() => onCancel(appointment)} className="px-4 py-2 text-[10px] font-black text-red-600 border border-red-500/20 hover:bg-red-500 hover:text-white rounded-sm uppercase">
                Hủy lịch
              </button>
              {type === "online" && (
                <a href={appointment.link || "#"} target="_blank" rel="noreferrer" 
                   className="flex items-center gap-2 px-6 py-2.5 bg-blue-600 text-white text-[10px] font-black uppercase rounded-sm hover:bg-blue-500">
          ...
          {status === "pending" && (
            role === "tutor" ? (
              <Link
                to="/tutor/myschedule"
                className="px-5 py-2.5 bg-blue-600 text-white text-[10px] font-black uppercase rounded-sm hover:bg-blue-500 flex items-center gap-2"
              >
                <Clock size={14} />
                Xem lịch
              </Link>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => onCancelBeforeAccept(appointment)} className="px-4 py-2.5 text-[10px] font-black text-red-500 border border-red-500/20 hover:bg-red-500 hover:text-white rounded-sm uppercase">
                  Hủy yêu cầu
                </button>
              </div>
            )
          ...
          {/* COMPLETED → merge logic student */}
          {status === "completed" && (
            <div className="flex gap-2 items-center">
              {/* Nút biên bản (giữ icon document từ student) */}
              <button onClick={() => onReport?.(appointment)} className="p-2.5 hover:bg-blue-500/10 text-slate-400 hover:text-blue-400">
                <FileText size={20} />
              </button>

              {/* Rating giữ nguyên theo role */}
              {role === "student" && (
                appointment.rating ? (
                  <div className="px-4 py-2 border border-emerald-500/30 text-emerald-500 text-[10px] font-black uppercase">
                    Đã đánh giá: {appointment.rating}
                  </div>
                ) : (
                  <button onClick={() => onFeedback(appointment)} className="px-6 py-2.5 bg-amber-500 text-amber-950 text-[10px] font-black uppercase rounded-sm hover:bg-amber-400">
                    Đánh giá
                  </button>
                )
              )}
          ...
          {/* CANCELLED / DECLINED → merge delete từ student */}
          {isCancelled && (
            <div className="flex gap-3 items-center">
              <button onClick={() => handleDeleteHistory()} className="p-2.5 text-slate-500 hover:text-red-500">
                <Trash size={18} />
              </button>
